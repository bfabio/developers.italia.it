...
function esDevelopersItaliaManager(queryType, config, objectConfig, params) {
  var defaultConfig = {
    ...
    'filterKeysMobileSelectors': {
      'publiccode.intendedAudience.scope': 'pills-scopes',
...
function esDevelopersItaliaQuery(config, params) {
  ...
  this.templates = {};
  this.templates.message = require('./message.handlebars');
  this.templates.tot = require('./tot.handlebars');
  this.templates.pager = require('./pager.handlebars');
  this.templates.search = require('./search.handlebars');
  this.templates.suggestion = require('./suggestion.handlebars');
  this.templates.suggestion_search_all = require('./suggestion_search_all.handlebars');
  this.templates.empty = require('./empty.handlebars');
...
esDevelopersItaliaQuery.prototype.esSearchSuccessCallback = function (response) {
  ...
  if (response.hits.total === 0) {
    ...
    var language = this.config['language'];
    var $intro = $('.intro > h1');
    const sanitized = $('<div>').text(keyword.split('+').join(' ')).html();
    $intro.html(
...
esDevelopersItaliaAutocompleteAllQuery.prototype.esSearchSuccessCallback = function (response) {
  ...
  if ((typeof response.hits !== 'undefined') && (typeof response.hits.hits !== 'undefined') && Array.isArray(response.hits.hits)) {
    ...
    $suggestions.append(this.templates.suggestion_search_all({
